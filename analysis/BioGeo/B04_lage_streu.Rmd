...
Wir benutzen den `palmerpenguins` - Datensatz, um das durchzuexerzieren und müssen ihn deshalb in R laden: einmal das Paket und dann die Daten:
...
Mit einem `<-` wird ein Wert, den ich berechne einer Variablen zugewiesen, mit der ich später weiterrechnen kann. Das ist häufig eine gute Idee. Das `$`-Zeichen bezeichnet den vector ("die Spalte") eines data frames ("der Tabelle"). Also `penguins$body_mass_g` ist die Spalte `body_mass_g` in der Tabelle `penguins`. Wir müssen R immer sagen, wie er mit fehlenden Werten im Datensatz umgehen soll. Mit fehlenden Werten kann R nicht rechnen, deswegen sagen wir in jedem dieser Befehle `na.rm = TRUE`, d.h. "ja (= TRUE, es ist wahr), entferne (remove) alls NAs".

Wir berechnen so die Lagemaße des Gewichts der Pinguine:

```{r Lagemaße, eval=FALSE}
# Arithmetisches Mittel
body_mass_g_mean <- mean(penguins$body_mass_g, na.rm = TRUE) 
body_mass_g_mean

# Median
body_mass_g_med <- median(penguins$body_mass_g,  na.rm = TRUE) 
body_mass_g_med

# Häufigkeitstabelle
tab <- table(penguins$body_mass_g) # wie häufig kommt jeder Wert vor? 
tab 

# Standardabweichung
body_mass_g_sd <- sd(penguins$body_mass_g,  na.rm = TRUE)
body_mass_g_sd

# Varianz
body_mass_g_var <- var(penguins$body_mass_g,  na.rm = TRUE)
body_mass_g_var

# Spannweite
body_mass_g_range <- range(penguins$body_mass_g,  na.rm = TRUE) 
body_mass_g_range
```
...
```{r Exkurs: Modus-Funktion, eval=FALSE}
# Funktion schreiben
getmode <- function(v) { # Funktion getmode wird auf einen Vektor v angewendet
   uniqv <- unique(v) # Vektor mit den Werten von v, ohne Dopplungen 
   uniqv[which.max(tabulate(match(v, uniqv)))] # welcher maximale Wert entsteht, 
   # wenn ich zähle, wie häufig die Werte von uniqv in v vorkommen
   }

# Modus 
mod_body_mass_g <- getmode(penguins$body_mass_g)
mod_body_mass_g
```

**Aufgaben:** 

1. Überprüft doch einmal, ob ihr mit der getmode genau den gleichen Modus bekommt, wie ihr selber mit table gefunden habt! :-)

2. Berechnet die Lagemaße noch einmal für `penguins$flipper_length_mm`. 

3. Für welche Variablen im Datensatz können wir welche Lagemaße berechnen? Diskutiert mit eurer Gruppe!
...
**Aufgabe:** 
Schaut euch die eben berechnete Werte (Pinguingewicht und Flossenlänge) noch einmal an. Wie ist das Verhältnis von Median, Modus und Mittelwert? 

Wie stellt ihr euch die Verteilungen vor, nachdem ihr das und die Standardabweichungen berechnet habt? Diskutiert in der Gruppe!
...
Die Normalverteilung ist eine in der Statistik häufig zitierte Beschreibung einer bestimmten Form von Messwertreihe. Sie hat folgende Eigenschaften:
 ...
 - Mittelwert, Median und Modus sind identisch, liegen genau in der Mitte und teilen die Verteilung in zwei gleich große Hälften (weil es ja der Median ist).
...
```{r}
# Dataframe mit normalverteilten Daten erzeugen
normal <- data.frame(x = rnorm(100, mean = 5, sd = 2))
```

Schauen wir sie uns doch einmal genauer an! 

**Aufgabe** Bitte berechnet Mittelwert, Median, Modus, Standardabweichung und Varianz von `normal$x`.
Was hätten wir uns schon denken können?
