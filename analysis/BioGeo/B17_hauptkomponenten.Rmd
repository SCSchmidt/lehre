...
Die Hauptkomponentenanalyse ist in R base implementiert. Wir nutzen hier die Funktion `prcomp()`. 

Nehmen wir als Beispiel unsere Piraten und schauen, wie die metrischen Variablen `height`, `weight`, `beard.length` und `sword.time` mithilfe einer PCA auf zwei Faktoren reduziert werden können. 
...
Da der Datensatz sehr groß ist (eigentlich ja gut), für die Visualisierung das aber schwierig wird, ziehen wir eine Stichprobe:
...
```{r}
# Ergebnis PCA als Objekt definieren
pirates.pca <- prcomp(pir[,c(4:5,14:15)], # Auswahl der metrischen stetigen Spalten
                      center = TRUE, # zentrieren
                      scale. = TRUE) # normalisieren
```

Jetzt haben wir die PCA einem Objekt zugewiesen und können uns Details mit dem üblichen `summary` anzeigen lassen:

```{r}
# Zusammenfassung der Ergebnisse
sum <- summary(pirates.pca)
sum
```
...
Wir sehen also, dass die erste Hauptkomponente `r round(sum$importance[2, 1]*100, 0)`% der Varianz erklärt, die zweite erklärt `r round(sum$importance[2, 2]*100, 0)`% und zusammen erklären sie `r round(sum$importance[3, 2]*100, 0)`% der Varianz. 
...
Wir haben "nur" 4 PCs, d.h. principal compontents (Hauptkomponenten), weil wir 4 Variablen in die Analyse einbezogen haben. Wir sehen jetzt die Standardabweichung, Anteil der erklärten Varianz und "cumulative proportion", d.h. den Wert, wenn wir die Varianz dieser und aller "vorhergehenden Hauptkomponenten" zusammenrechnen. 
...
Das würden wir jetzt eigentlich gern visualisieren. Das "schönste" Paket dafür liegt unter Github und heißt `ggbiplot`. Wenn man sich ein Paket von Github installieren möchte, dann braucht man dafür das Paket `devtools`. Installiert Euch das wie immer und und holt euch dann `ggbiplot` von Github. 
...
```{r}
# Biplot mit Gruppenvariable erstellen
...
```{r}
# Biplot für den 3. und 4. Eigenvektor erstellen
