...
Wie immer installieren bitte nur diejenigen das Paket, die nicht in der Cloud arbeiten!
```{r, eval = FALSE}
...
Aus irgendwelchen Gründen wir bei der ANOVA der p-Wert nicht gleich angezeigt. Deswegen weisen wir das Ergebnis der eigentlichen Test-Berechnung einer Variablen zu und lassen uns dann das Ergebnis zusammengefasst ausgeben:
...
Erinnert ihr euch an das Thema "crossed" und "nested", das ich beim Chi-Quadrat-Test aufgeworfen hatte? Da bei diesem Datensatz alle Geschlechter in den Colleges vorkommen, haben wir ein "crossed design" (ein gekreuztes Design).
...
res.man <- manova(cbind(pirates$height, pirates$weight) ~ pirates$sex, data = pirates)
