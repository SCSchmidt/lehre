...
Der Test in R benutzt eine recht "alte" Syntax, die manchmal vorkommt. Mithilfe der Tilde "~" beschreibt er, welche zwei Variablen gegeneinander getestet werden sollen. `paired = FALSE` stellt sicher, dass wir hier den U-Test und nicht den Wilcoxon-Test berechnen.
...
Die Piraten haben eine Anzahl von Tatoos. Das ist nun eine eindeutig diskrete Variable, denn es wird wohl kaum jemand mit einem halben Tatoo herumlaufen. Trotzdem ist sie als `numeric` abgelegt, was gut ist, da der `wilcox.test` als Datentyp einen numerischen Vektor braucht.

Wir können jetzt schauen, ob die Zahl der Tatoos bei Piraten mit und ohne Bandana (`headband`) unterschiedlich ist. Das heißt:
