...
c <- subset(penguins, penguins$species == "Chinstrap")

f1 <- lm(bill_length_mm ~ bill_depth_mm, data = c)

f2 <- lm(bill_depth_mm ~ bill_length_mm, data = c)
...
ggplot(data = c,
       aes(x = bill_depth_mm,
           y = bill_length_mm))+
  geom_point()+
    geom_smooth(aes(x = bill_depth_mm, 
                    y = bill_length_mm), method = "lm",
              se = FALSE)+
  stat_poly_eq(aes(x = bill_depth_mm, 
                   y = bill_length_mm, 
                   label =  paste(..eq.label..,     # kopier die Formel (equation)
                                  ..adj.rr.label.., # kopier R²adjusted
                   sep = "~~~~")), # Trenn es mit 4 Leerzeichen
...
ggplot(data = c,
  ...
  geom_point()+
    geom_smooth(aes(x = bill_length_mm, 
                    y = bill_depth_mm ), method = "lm",
              se = FALSE)+
  stat_poly_eq(aes(x = bill_length_mm, 
                   y = bill_depth_mm, 
                   label =  paste(..eq.label..,     # kopier die Formel (equation)
                                  ..adj.rr.label.., # kopier R²adjusted
                   sep = "~~~~")), # Trenn es mit 4 Leerzeichen
...
```{r}
plot(f2, which=3, col=c("blue"))  # Scale-Location Plot
