...
Probiert einmal verschiedene Werte für die Klassengröße (binwidth) in dem Beispiel aus, z. B. einmal 5 und einmal 500:
...
Ich hatte ja schon erläutert, was ein Boxplotdiagramm ist. Hier noch einmal zur Erinnerung:
 ...
 - Q3 - Q1 ist der Interquartilsabstand: In diesem Bereich um den Median herum liegen 50% der "mittleren" Werte. Er wird durch die Box gekennzeichnet
 - Bartenden bzw. "Whisker" kennzeichen den geben den minimalen und den maximalen Wert an. 
 - *Aber:* Bei der Berechnung des Minimus und des Maximums bleiben Ausreißer unberücksichtigt. Ausreisser sind Werte, die mehr als das 1.5fache vom oberen bzw. unteren Quartil entfernt sind. So macht es zumindest ggplot, in anderen Programmen wird das anders definiert!
...
```{r ein_Boxplotdiagramm, fig.height=5, fig.width=5}
...
```{r StreudiagrammSchoenePunkten, fig.height=5, fig.width=5}
...
```{r Streudiagramm_Legendenbeschriftung, fig.height=5, fig.width=5}
...
Mit `scale_colour_discrete` kann ich Legenden (`scales`) verändern, die mit `color` innerhalb des aesthetics-Bereichs meines Codes für die Graphik definiert werden und die DISKRET sind (also in erster Linie nominale / ordinale Daten).
