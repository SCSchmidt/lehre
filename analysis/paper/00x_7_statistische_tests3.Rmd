...
Die Nullhypothese ist: Es gibt keinen Unterschied zwischen den beiden Gruppen.
Die Alternativhypothese ist: Es gibt einen Unterschied zwischen den beiden Gruppen.
...
Hier sagen wir: für jede Zahl i (* for (i in*) von 1 bis (*c(1:)*) "so viele wie Zeilen im Datensatz BACups sind" (*nrow(BACups) ))*) tu das Folgende (das was in geschweiften klammern steht): 
Finde den größten Wert (*max()*) für die Zeile i in den Spalten RD, SD und ND heraus (*jeweils [i]*) und schreibe ihn in der Zeile i in die Spalte maxD hinein (* BACups$maxD[i] <- *).
