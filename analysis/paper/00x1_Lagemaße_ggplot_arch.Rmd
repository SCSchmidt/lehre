...
Als nächstes visualisieren wir mit diesem Code die Häufigkeiten, wie oft die verschiedenen Basenformen (Base.Sh) in den unterschiedlichen Spitzen-Typen vorkommen (Name).
...
Mit `labels = ` haben wir R mitgeteilt, wie wir die in alphabetischer Reihenfolge gegebenen Werte neu benennen möchten. Das `c()` hat R angezeigt, dass sie alle zusammen gehören und kein neuer Befehl gestartet wurde. Text muss in Hochkommas geschrieben werden!
...
1. Überlegt bitte, (und diskutiert evtl mit eurer Gruppe) was in dem nächsten Code Chunk passiert. Die Hilfe kann mit ?Suchbegriff abgerufen werden.
...
2. Erstellt ein neues Säulendiagramm
